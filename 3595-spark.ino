...
#define LCD_CLK 	D0	//(1<<PB0)
#define LCD_SIO		D1	//(1<<PB1)
#define LCD_CS		D2	//(1<<PB2)
#define LCD_RST		D3	//(1<<PB3)
...
void LCD_Out(unsigned char Data, unsigned char isCmd) {
    if(isCmd) digitalWrite(LCD_CS, HIGH); 
    digitalWrite(LCD_CLK, LOW);
    digitalWrite(LCD_CS, LOW);  //Clock and CS low

    digitalWrite(LCD_SIO, HIGH);        //SData High
    if(isCmd) digitalWrite(LCD_SIO, LOW); //If it is a command, SData Low

    digitalWrite(LCD_CLK, HIGH);        //Clock High

    for(char x=0; x<8; x++)    {
        digitalWrite(LCD_SIO, LOW);
        digitalWrite(LCD_CLK, LOW);        //Clock and SData low
        if(Data & 128) digitalWrite(LCD_SIO, HIGH);      // Mask MSB - SData high if it is a 1
        digitalWrite(LCD_CLK, HIGH);            //Clock High
...
*/
  ...
  //Booster Voltage On
  LCD_Out(0x03, 1);
  delay(50);  //NOTE: At least 40ms must pass between voltage on and display on.
